...
for i=1:max(size(db_fam))
  [phi,psi,xval]=wavefun(db_fam(i),ITER);
  assert_checkalmostequal ( sum(phi)/(2^ITER) , 1 , %eps, 1e-7 );
  assert_checkalmostequal ( sum(psi.^2)/(2^ITER) , 1 , %eps, 1e-7  );
  assert_checkalmostequal ( sum(psi) , 0 , %eps, 1e-10 );
//   tmp=fft(psi);
...
for i=1:max(size(coif_fam))
  ...
  [phi,psi,xval]=wavefun(coif_fam(i),ITER);
  assert_checkalmostequal ( sum(phi)/(2^ITER) , 1 , %eps, 1e-7 );
  assert_checkalmostequal ( sum(psi.^2)/(2^ITER) , 1 , %eps, 1e-7  );
  assert_checkalmostequal ( sum(psi) , 0 , %eps, 1e-5 );
//   tmp=fft(psi);
//   assert_checkalmostequal ( real(tmp(1)) , 0 , %eps,1e-3);
end;
...
for i=1:max(size(sym_fam))
  [phi,psi,xval]=wavefun(sym_fam(i),ITER);
  assert_checkalmostequal ( sum(phi)/(2^ITER) , 1 , %eps, 1e-7 );
  assert_checkalmostequal ( sum(psi.^2)/(2^ITER) , 1 , %eps, 1e-7  );
